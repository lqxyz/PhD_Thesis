...
\end{equation}
where LWP is the liquid water path (g m$^{-2}$, defined in \Eqref{eq:cwp}) and $r_e$ is effective radius in units of micrometers ($\mu$m). Therefore in this study, the cloud optical properties are parameterized by effective radius (\secref{sec:effective_radius}) and in-cloud water content (\secref{sec:in_cloud_water}).

\subsection{Effective radius}
\label{sec:effective_radius}
% 2.53 of \cite{Hansen1974light}
