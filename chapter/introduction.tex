...
As shown in \figref{fig:earth_energy_budget}, clouds can exert strong and competing influences on the energy budget of the Earth. In general, we quantify the impact of clouds on planet's radiation budget using cloud radiative effect (CRE), which is defined as the differences in TOA net radiative fluxes between all-sky and clear-sky conditions, or the outgoing (or reflected) radiative fluxes at TOA between clear-sky and all-sky conditions \citep[e.g.,][]{Ramanathan1989,Li2017}. Note that CRE is also called cloud radiative forcing (CRF) in some literatures, and in this study we prefer using CRE.
...
Specifically, clouds cool the Earth by reflecting the incoming shortwave radiation emitted by the sun.
