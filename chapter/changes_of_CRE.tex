...
In the AMIP-type simulations or the ones performed with Q-flux (to be introduced in \secref{sec:Q_flux_intro}, the COSP is only run for the last five years, and only the ISCCP simulator is enabled. One key diagnostic output to be acquired from the COSP is the  joint histogram of cloud-top pressure (CTP) and optical depth. In current setup, the histogram segments are obtained separately first, and then the whole histogram is derived directly by concatenating these segments, as they are independent to each other.
