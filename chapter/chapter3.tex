...
We find that the vertical structure of clouds can be simulated relatively well without explicit diagnosis of convective clouds (see \figref{fig:cld_fraction_profile}), and we leave the possible explicit representation of convective clouds to a future study. 
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{{figs/chp3/fit_linear_coeff_profile}.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{{figs/chp3/cmp_sundqvist_linear_with_ecmwf_cc_t_12_lev_20}.png}
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{{figs/chp3/feezedry_with_various_qv}.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{{figs/chp3/low_cld_frac_vs_diff_vars_season}.pdf}
...
\begin{table}
	...
	\resizebox{\textwidth}{!}{ 
		...
		\begin{tabular}{c c p{4.2cm} p{5.5cm} l} 
			\hline
			...
			\hline
			\multirow{2}{*}{$q_l$} & \multirow{2}{*}{g kg$^{-1}$} & In-cloud liquid water mixing ratio & $\max\left(3\times 10^{-4}, ~q_{l0}\times \min\left(1, ~\frac{T-220}{280-220}\right)\right)$, ($T$ in units of K) & \multirow{2}{*}{$q_{l0}$=0.18 g kg$^{-1}$} \\
			\hline
		\end{tabular}
	}
