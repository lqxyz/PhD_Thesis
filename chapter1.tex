...
Noting that zonal-mean variables, not the global-mean values, will be applied in Eq. \eqref{eq:lambda_def} so we can analyze local or regional feedback, as it offers some advantages such as spatial pattern of changes \citep{Feldl2013,Feldl2017}.
...
and $(\delta \ln e_s)_i$ is estimated by $\left(e_s(T_i+1)-e_s(T_i)\right)/ e_s(T_i)$, which is similar to the method that \cite{Pendergrass2018} used to calculate the water vapor kernel, and hence we use Eq. \eqref{eq:fb_wv_YHuang} to calculate the water vapor kernel in this study.
...
\begin{figure}[tbhp]
	\centering
	\includegraphics[width=1\textwidth]{../../Exps_Analysis/radiative_kernels/feedback_analysis/figs/all_zonalmean_feedbacks}
